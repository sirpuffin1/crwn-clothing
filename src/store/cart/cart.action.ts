...
export const clearCart = (cartItems: CartItem[]) => {
  const newCartItems = clearItems(cartItems);
  return setCartItems(newCartItems);
};
...
const clearItems = (cartItems: CartItem[]): CartItem[] => {
  return cartItems.filter((cartItem) => cartItem.id === 9090);
};
